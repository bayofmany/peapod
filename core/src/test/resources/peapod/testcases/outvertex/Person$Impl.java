...
public final class Person$Impl extends Person
    ...
    public java.util.List<Person> getKnows() {
        return java.util.Collections.unmodifiableList(v.out("knows").map(v -> (peapod.testcases.outvertex.Person) new peapod.testcases.outvertex.Person$Impl(v.get(), graph)).toList());
    }
